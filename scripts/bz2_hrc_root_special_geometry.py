...
def _decode_class1_grid(data: bytes, outer: dict) -> dict | None:
    ...
    if primitive_kind != 2 or not (2 <= u_count <= 4096 and 2 <= v_count <= 4096):
        return None
    vertex_count = u_count * v_count
    end = offset + 6 + vertex_count * 12
    if vertex_count > 2_000_000 or end > len(data):
        return None
    values = struct.unpack_from(f">{vertex_count * 3}f", data, offset + 6)
    vertices = [tuple(values[index : index + 3]) for index in range(0, len(values), 3)]
    if not all(math.isfinite(value) and abs(value) < 1.0e9 for point in vertices for value in point):
        return None
    ...
    # Row-major lattice. This winding points the planar reference grids toward +Y.
